...
import { Plus, PencilSquare, Trash, Eye, ArrowRightCircle } from "react-bootstrap-icons";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import BreadCrumb from "../../../components/BreadCrumb";
import ProformaBillModal from "./ProformaBillModal";
import proformaBillService from "../../../services/proformaBillService";
...
const ProformaBills = () => {
  ...
  const [proformaBills, setProformaBills] = useState([]);
  const [branches, setBranches] = useState([]);
  const [showModal, setShowModal] = useState(false);
  const [selectedProformaBill, setSelectedProformaBill] = useState(null);
  ...
  const [previewProformaBill, setPreviewProformaBill] = useState(null);
  ...
  const normalizeProformaBill = (proformaBill) => {
    const services = (proformaBill.services || []).map((service) => {
      ...
      if (taxRate > 0 && cgst === 0 && sgst === 0 && igst === 0) {
        if (Number(service.igst_amount) > 0) {
          ...
          const stateStr = String(proformaBill.buyer_state_code || proformaBill.consignee_state_code || proformaBill.buyer_state || proformaBill.consignee_state || "").toLowerCase();
  ...
  const fetchProformaBills = async () => {
    try {
      const res = await proformaBillService.getAllProformaBills();
      const normalizedBills = (res?.success ? res.data : res) || [];
      setProformaBills(normalizedBills);
      ...
      normalizedBills.forEach(async (bill) => {
        try {
          ...
          const res = await proformaBillService.getPaymentsByBill(billId);
          const payments = res?.data || [];
          
          const calculatedSummary = payments.reduce((acc, p) => {
            ...
            return {
              ...
              total_tds: acc.total_tds + tds,
              total_deductions: acc.total_deductions + deductions,
              total_settled: acc.total_settled + received + tds + deductions
            };
          }, { total_received: 0, total_tds: 0, total_deductions: 0, total_settled: 0 });
          
          const totalAmount = (bill.services || []).reduce((sum, s) => {
            const total = Number(s.total_amount != null ? s.total_amount : 0);
            return sum + (total || (Number(s.rate || 0) * Number(s.quantity || 0)));
          }, 0) || 0;
          
          calculatedSummary.bill_total = totalAmount;
          calculatedSummary.advance_amount = Number(res?.summary?.advance_amount || bill.advance_amount || 0);
          calculatedSummary.pending_amount = totalAmount - calculatedSummary.total_settled - calculatedSummary.advance_amount;

          setListPaymentSummaries(prev => ({
            ...prev,
            [billId]: calculatedSummary
          }));
        } catch (error) {
          console.error(`Failed to fetch summary for bill ${bill.id || bill._id}`);
        }
      });
    } catch (error) {
      console.error("Error fetching proforma bills:", error);
      toast.error("Failed to fetch proforma bills");
    }
  };
  ...
  const openEdit = async (proformaBill) => {
    try {
      const res = await proformaBillService.getProformaBillById(proformaBill.id);
      const billData = res?.data || res;
      setSelectedProformaBill(billData);
      setFormData(normalizeProformaBill(billData));
      setShowModal(true);
    } catch (error) {
      console.error("Error loading proforma bill for edit:", error);
      toast.error("Failed to load proforma bill details for editing");
    }
  };

  const openPreview = async (proformaBill) => {
    try {
      const res = await proformaBillService.getProformaBillById(proformaBill.id);
      const billData = res?.data || res;
      setPreviewProformaBill(normalizeProformaBill(billData));
      setShowPreviewModal(true);
    } catch (error) {
      console.error("Error loading proforma bill for preview:", error);
      toast.error("Failed to load proforma bill details for preview");
    }
  };

  const handleConvertToSaleBill = (proforma) => {
    // Navigate to Create Sale Invoice page and pass proforma data to pre-fill the form
    navigate("/works/salebills/create", { state: { proformaData: proforma } });
  };
  ...
  const handleSave = async (payload) => {
    ...
    try {
      if (selectedProformaBill) {
        await proformaBillService.updateProformaBill(selectedProformaBill.id, updatedPayload);
        toast.success("Proforma bill updated successfully");
      } else {
        await proformaBillService.createProformaBill(updatedPayload);
        toast.success("Proforma bill created successfully");
      }
      setShowModal(false);
      fetchProformaBills();
    } catch (error) {
      console.error("Error saving proforma bill:", error);
      toast.error("Failed to save proforma bill");
    }
  };

  const openPaymentModal = async (proformaBill) => {
    setActivePaymentBill(proformaBill);
    setPaymentFormData({
    ...
    try {
      const billId = proformaBill.id || proformaBill._id;
      const res = await proformaBillService.getPaymentsByBill(billId);
      const payments = res?.data || [];
      setBillPayments(payments);
      
      const totalAmount = (proformaBill.services || []).reduce((sum, s) => {
        const total = Number(s.total_amount != null ? s.total_amount : 0);
        return sum + (total || (Number(s.rate || 0) * Number(s.quantity || 0)));
      }, 0) || 0;
      ...
      calculatedSummary.bill_total = totalAmount;
      calculatedSummary.advance_amount = Number(res?.summary?.advance_amount || proformaBill.advance_amount || 0);
      calculatedSummary.pending_amount = totalAmount - calculatedSummary.total_settled - calculatedSummary.advance_amount;
      ...
      setPaymentSummary({
        ...
        total_received: Number(proformaBill.received_amount || 0),
        total_tds: Number(proformaBill.tds || 0),
        total_deductions: Number(proformaBill.deductions || 0),
        advance_amount: Number(proformaBill.advance_amount || 0),
        pending_amount: totalAmount - Number(proformaBill.received_amount || 0),
      });
      setBillPayments([]);
    } finally {
  ...
  const handlePaymentSave = async () => {
    ...
    try {
      if (paymentFormData.id) {
        await proformaBillService.updatePayment(paymentFormData.id, payload);
        toast.success("Payment updated successfully");
      } else {
        await proformaBillService.createPayment(payload);
        toast.success("Payment added successfully");
      }
      
      // Refresh payments list and summary
      const res = await proformaBillService.getPaymentsByBill(activePaymentBill.id);
      const payments = res?.data || [];
      setBillPayments(payments);
      
      const totalAmount = (activePaymentBill.services || []).reduce((sum, s) => {
        const total = Number(s.total_amount != null ? s.total_amount : 0);
        return sum + (total || (Number(s.rate || 0) * Number(s.quantity || 0)));
      }, 0) || 0;
      ...
      calculatedSummary.bill_total = totalAmount;
      calculatedSummary.advance_amount = Number(res?.summary?.advance_amount || activePaymentBill.advance_amount || 0);
      calculatedSummary.pending_amount = totalAmount - calculatedSummary.total_settled - calculatedSummary.advance_amount;
      
      setPaymentSummary(calculatedSummary);
      fetchProformaBills(); // Refresh main list too
  ...
  const handleEditPayment = (payment) => {
    setPaymentFormData({
      id: payment.id,
      proforma_bill_id: payment.proforma_bill_id,
  ...
  const handleDeletePayment = async (paymentId) => {
    ...
    try {
      await proformaBillService.deletePayment(paymentId);
  ...
  const handleDownloadExcel = () => {
    if (!filteredProformaBills.length) {
      toast.warning("No proforma bills to export");
      return;
    }
    ...
    // Header Row 1: Title
    wsData.push(["Proforma Invoice Report"]);
    ...
    XLSX.writeFile(wb, `Proforma_Invoices_${moment().format("YYYYMMDD")}.xlsx`);
  };

  const handleDelete = (id) => {
    ConfirmDeleteModal({
      title: "Delete Proforma Bill",
      message: "This action cannot be undone. Continue?",
      iconColor: "#ff0000",
      onConfirm: async () => {
        await proformaBillService.deleteProformaBill(id);
        toast.success("Proforma bill deleted successfully");
        fetchProformaBills();
      },
    });
  };

  const filteredProformaBills = (proformaBills || []).filter((item) => {
  ...
  const generatePDF = (bill) => {
    ...
    doc.save(`ProformaBill_${bill.invoice_number || Date.now()}.pdf`);
  };

  return (
    <div className="page-content">
      <h4 className="mb-1">Proforma Invoice</h4>
      <BreadCrumb pathname={location?.pathname || ""} lastLabel="Proforma Invoice" />
      ...
      <Card>
        <Card.Body>
          <div className="d-flex flex-wrap align-items-end gap-3 mb-4 pb-3 border-bottom">
            ...
            <div className="ms-auto d-flex gap-2">
              ...
              <Button variant="primary" onClick={openCreate} className="px-3">
                <Plus size={16} /> Create Proforma Invoice
              </Button>
            </div>
          </div>
            ...
            <Table striped hover bordered>
              ...
              <tbody>
                {startDateForOB && currentPage === 1 && (
                  <tr className="fw-bold" style={{ backgroundColor: '#f8f9fa' }}>
                    <td></td>
                    <td colSpan="6" className="text-primary" style={{ fontStyle: "italic" }}>Opening Balance</td>
                    <td className="text-danger">₹{(Math.round(openingBalance) || 0).toLocaleString("en-IN")}</td>
                    <td></td>
                  </tr>
                )}
                {filteredProformaBills
                  .slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
                  .map((sb, index) => {
                    const actualIndex = (currentPage - 1) * itemsPerPage + index + 1;
                    const totalAmount = Number(sb.services?.reduce((sum, s) => {
                      const total = Number(s.total_amount != null ? s.total_amount : 0);
                      return sum + (total || (Number(s.rate || 0) * Number(s.quantity || 0)));
                    }, 0) || 0);
 
                    // Use real-time summary if fetched, otherwise fallback to bill data
                    const billId = sb.id || sb._id;
                    const summary = listPaymentSummaries[billId];
                    const received = Number(summary?.total_received || sb.received_amount || sb.total_received || 0); 
                    const tds = Number(summary?.total_tds || sb.tds || sb.total_tds || 0);
                    const deduction = Number(summary?.total_deductions || sb.deductions || sb.total_deductions || 0);
                    const advance = Number(summary?.advance_amount || sb.advance_amount || 0);
                    
                    // Always use frontend calculated totalAmount as the base for difference
                    const difference = totalAmount - received - tds - deduction - advance;
                    ...
                    return (
                      <tr key={sb.id || actualIndex}>
                        ...
                        <td>
                          <span className={`badge bg-${sb.status === "paid" || sb.status === "settled" ? "success" : "danger"}`}>
                            {sb.status === "paid" || sb.status === "settled" ? "paid" : "pending"}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="d-flex gap-1">
                            <Button variant="outline-success" size="sm" onClick={() => handleConvertToSaleBill(sb)} title="Convert to Sale Bill">
                ...
                {filteredProformaBills.length === 0 && (
                  <tr>
              ...
              <tfoot className="table-light fw-bold">
                <tr>
                  ...
                  <td>
                    ₹{(filteredProformaBills.reduce((sum, sb) => {
                  ...
                  <td className="text-muted">
                    ...
                    ₹{((filteredProformaBills.reduce((sum, sb) => {
                      const total = Number((sb.services || []).reduce((sSum, s) => sSum + Number(s.total_amount || (Number(s.rate || 0) * Number(s.quantity || 0))), 0) || 0);
                      const billId = sb.id || sb._id;
                      const summary = listPaymentSummaries[billId];
                      const received = Number(summary?.total_received || sb.received_amount || sb.total_received || 0);
                      const tds = Number(summary?.total_tds || sb.tds || sb.total_tds || 0);
                      ...
                      const diff = total - received - tds - deduction - advance;
          ...
          {filteredProformaBills.length > itemsPerPage && (
            <div className="d-flex justify-content-center mt-3">
              <Pagination>
                <Pagination.First onClick={() => setCurrentPage(1)} disabled={currentPage === 1} />
                <Pagination.Prev onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} />
                
                {[...Array(Math.ceil(filteredProformaBills.length / itemsPerPage))].map((_, i) => (
                  <Pagination.Item
                    key={i + 1}
                ...
                )).slice(Math.max(0, currentPage - 3), Math.min(Math.ceil(filteredProformaBills.length / itemsPerPage), currentPage + 2))}

                <Pagination.Next
                  onClick={() => setCurrentPage((p) => Math.min(Math.ceil(filteredProformaBills.length / itemsPerPage), p + 1))}
                  disabled={currentPage === Math.ceil(filteredProformaBills.length / itemsPerPage)}
                />
                <Pagination.Last
                  onClick={() => setCurrentPage(Math.ceil(filteredProformaBills.length / itemsPerPage))}
                  disabled={currentPage === Math.ceil(filteredProformaBills.length / itemsPerPage)}
                />
              </Pagination>
            </div>
      ...
      <ProformaBillModal
        ...
        onSave={handleSave}
        isEdit={!!selectedProformaBill}
      />

      {/* Preview Modal - Exact match to image format */}
      <Modal size="xl" centered dialogClassName="modal-dialog-centered" className="proformaBill-preview-center" show={showPreviewModal} onHide={() => setShowPreviewModal(false)} backdrop="static" keyboard={false}>
        <Modal.Header closeButton>
          <div className="d-flex w-100 justify-content-between align-items-center">
            <h5 className="mb-0">Tax Invoice Preview</h5>
            <Button variant="primary" onClick={() => previewProformaBill && generatePDF(previewProformaBill)}>
              Download PDF
            </Button>
          </div>
        </Modal.Header>
        <Modal.Body style={{ backgroundColor: "#f5f5f5", padding: "30px", minHeight: "80vh" }}>
          {previewProformaBill ? (
            <div style={{ 
              ...
              <div style={{ marginBottom: "15px" }}>
                <table style={{ width: "100%", fontSize: "11px" }}>
                  <tbody>
                    <tr>
                      <td style={{ width: "25%" }}><strong>Invoice No :</strong> {previewProformaBill.invoice_number || "-"}</td>
                      <td style={{ width: "25%" }}><strong>Date :</strong> {previewProformaBill.invoice_date ? new Date(previewProformaBill.invoice_date).toLocaleDateString("en-IN") : "-"}</td>
                      <td style={{ width: "25%" }}><strong>Status :</strong> {previewProformaBill.status || "-"}</td>
                      <td style={{ width: "25%" }}><strong>IRN :</strong> {previewProformaBill.irn || "-"}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}>
                <div style={{ flex: 1, border: "1px solid #ddd", padding: "8px" }}>
                  ...
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>Name :</strong> {previewProformaBill.consignee_name || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>GSTIN :</strong> {previewProformaBill.consignee_gstin || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>Address :</strong> {previewProformaBill.consignee_address || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>State :</strong> {previewProformaBill.consignee_state || "-"} ({previewProformaBill.consignee_state_code || "-"})</p>
                </div>
                <div style={{ flex: 1, border: "1px solid #ddd", padding: "8px" }}>
                  <h6 style={{ fontWeight: "bold", marginBottom: "5px" }}>Buyer (Bill to)</h6>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>Name :</strong> {previewProformaBill.buyer_name || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>GSTIN :</strong> {previewProformaBill.buyer_gstin || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>Address :</strong> {previewProformaBill.buyer_address || "-"}</p>
                  <p style={{ margin: 0, fontSize: "10px" }}><strong>State :</strong> {previewProformaBill.buyer_state || "-"} ({previewProformaBill.buyer_state_code || "-"})</p>
                </div>
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10px", marginBottom: "15px" }}>
                ...
                <tbody>
                  {(previewProformaBill.services || []).map((service, idx) => {
                    const quantity = Number(service.quantity || 0);
                    const rate = Number(service.rate || 0);
              ...
              <div style={{ marginBottom: "15px" }}>
                <p style={{ fontSize: "11px", margin: 0 }}>
                  <strong>Amount Chargeable (in words):</strong> {numberToWords((previewProformaBill.services || []).reduce((sum, s) => sum + (Number(s.quantity || 0) * Number(s.rate || 0)), 0) + (previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.tax_amount || 0), 0))}
                </p>
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10px", marginBottom: "15px" }}>
                ...
                <tbody>
                  ...
                  <tr style={{ fontWeight: "bold" }}>
                    ...
                    <td style={{ border: "1px solid #000", padding: "6px", textAlign: "right" }}><strong>₹ {(previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.amount || 0), 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></td>
                    <td style={{ border: "1px solid #000", padding: "6px" }}></td>
                    <td style={{ border: "1px solid #000", padding: "6px", textAlign: "right" }}><strong>₹ {((previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.amount || 0), 0) + (previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.tax_amount || 0), 0)).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></td>
                    <td style={{ border: "1px solid #000", padding: "6px" }}></td>
                    <td style={{ border: "1px solid #000", padding: "6px" }}></td>
                    <td style={{ border: "1px solid #000", padding: "6px", textAlign: "right" }}><strong>₹ {(previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.tax_amount || 0), 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></td>
                  </tr>
                </tbody>
              </table>
              ...
              <div style={{ marginBottom: "15px" }}>
                <p style={{ fontSize: "11px", margin: 0 }}>
                  <strong>Tax Amount (in words):</strong> {numberToWords((previewProformaBill.services || []).reduce((sum, s) => sum + Number(s.tax_amount || 0), 0))}
                </p>
              </div>
              ...
              <div style={{ marginBottom: "10px" }}>
                <p style={{ fontSize: "10px", margin: 0 }}>
                  <strong>BANK DETAILS:</strong> {previewProformaBill.bank_name || "AXIS BANK LIMITED"}, A/C NO. {previewProformaBill.account_number || "911020042168303"}, IFSC: {previewProformaBill.ifsc_code || "UTIB0000550"}, BRANCH {previewProformaBill.bank_branch || "BIDANASI, CUTTACK, ODISHA"}.
                </p>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: "15px", borderTop: "1px solid #ddd", paddingTop: "8px" }}>
                ...
                <p style={{ fontSize: "9px", margin: 0 }}><strong>Company's PAN:</strong> {previewProformaBill.company_pan || "ARXPK7658Q"}</p>
              </div>
            </div>
          ) : (
              ...
              <div className="p-3 bg-light rounded border d-flex justify-content-between align-items-center">
                ...
                <div className="text-end">
                  ...
                  <h5 className="mb-0 text-danger">
                    ₹{(Math.round(Number(paymentSummary.bill_total || 0) - Number(paymentSummary.total_received || 0) - Number(paymentSummary.total_tds || 0) - Number(paymentSummary.total_deductions || 0) - Number(paymentSummary.advance_amount || 0)) || 0).toLocaleString("en-IN")}
                  </h5>
                </div>
              </div>
            </Col>
